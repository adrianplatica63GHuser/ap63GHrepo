...
async function fetchPaperworkPersons(paperworkId: string): Promise<AssociatedPerson[]> {
...
export function PaperworkPersonsTab({ paperworkId }: Props) {
  const t           = useTranslations("paperwork.persons");
  ...
  const { data: items, isLoading, isError } = useQuery({
    queryKey: ["paperwork-persons", paperworkId],
    queryFn:  () => fetchPaperworkPersons(paperworkId),
  });

  const handleAssociate = () => {
    router.push(`/paperwork/${encodeURIComponent(paperworkId)}/associate-person`);
  };
  ...
  const handleDissociate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/paperwork/${encodeURIComponent(paperworkId)}/persons/${encodeURIComponent(selectedId)}`,
        { method: "DELETE" },
      );
      if (!res.ok) {
