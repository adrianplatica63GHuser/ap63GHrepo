...
async function fetchParties(paperworkId: string): Promise<PartyItem[]> {
...
export function SuccessionPartiesPanel({ paperworkId, mode }: Props) {
  const t           = useTranslations("paperwork.successionParties");
  ...
  const { data: items, isLoading, isError } = useQuery({
    ...
    queryFn:  () => fetchParties(paperworkId),
  });

  const handleAddParty = () => {
    router.push(`/paperwork/${encodeURIComponent(paperworkId)}/associate-party`);
  };
  ...
  const handleRemove = async (personId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/paperwork/${encodeURIComponent(paperworkId)}/persons/${encodeURIComponent(personId)}`,
        { method: "DELETE" },
      );
      if (!res.ok) {
