...
async function searchProperties(q: string): Promise<PropertySearchItem[]> {
  ...
  return data.items as PropertySearchItem[];
}

export function AssociatePropertyView({ paperworkId, paperworkName }: Props) {
  ...
  const { data: results, isLoading, isError } = useQuery({
    queryKey: ["property-search-pw", searchInput],
    queryFn:  () => searchProperties(searchInput),
  });
  ...
  const handleAssociate = async () => {
    ...
    try {
      ...
      router.push(`/paperwork/${encodeURIComponent(paperworkId)}?tab=properties`);
    } catch (err) {
