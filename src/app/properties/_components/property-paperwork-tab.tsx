...
async function fetchPropertyPaperwork(propertyId: string): Promise<AssociatedPaperwork[]> {
...
export function PropertyPaperworkTab({ propertyId }: Props) {
  const t           = useTranslations("property.paperwork");
  ...
  const { data: items, isLoading, isError } = useQuery({
    queryKey: ["property-paperwork", propertyId],
    queryFn:  () => fetchPropertyPaperwork(propertyId),
  });

  const handleAssociate = () => {
    router.push(`/properties/${encodeURIComponent(propertyId)}/associate-paperwork`);
  };
  ...
  const handleDissociate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/properties/${encodeURIComponent(propertyId)}/paperwork/${encodeURIComponent(selectedId)}`,
        { method: "DELETE" },
      );
      if (!res.ok) {
