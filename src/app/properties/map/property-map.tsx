...
export default function PropertyMap() {
  ...
  const toggleSelectMode = useCallback(() => {
    ...
    // Always reset selection when toggling
    setSelectedIds(new Set());
    setDragStart(null);
    setDragCurrent(null);
  }, []);
  ...
  const handleOverlayMouseLeave = useCallback(() => {
    // Abort an in-progress drag if the cursor leaves the map area
    if (dragStart) {
      setDragStart(null);
      setDragCurrent(null);
    }
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      setShowDeleteConfirm(false);
      setSelectMode(false);
      // Refetch the map and any list views
      await queryClient.invalidateQueries({ queryKey: ["properties"] });
    } catch (err) {
      setDeleteError(
        err instanceof Error ? err.message : "An unexpected error occurred",
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      ...
      <Map
        ...
        mapTypeId={mapType}
        disableDefaultUI
        // Disable all gestures while the selection overlay is active so the
        // user's drag draws a rectangle instead of panning the map.
        gestureHandling={selectMode ? "none" : "greedy"}
        style={{ width: "100%", height: "100%" }}
        onClick={() => { if (!selectMode) setSelected(null); }}
      >
        {/* Inner helpers that need useMap() / useMapsLibrary() */}
        <MapRefCapture mapRef={mapRef} />
        <FitAllProperties items={withGeometry} />
        ...
        {/* ── Corner markers (second pass, constant-size red dots) ────────── */}
        {withGeometry.flatMap((prop) => {
          const label = prop.nickname ?? prop.code;
          const pos   = centroid(prop.corners);
          return prop.corners.map((corner, idx) => (
      ...
      {/* ------------------------------------------------------------------ */}
      {/*
        Sits exactly on top of the map (absolute inset-0, z-10).
        pointer-events: none while in pan mode → map works normally.
        pointer-events: all while in select mode → captures the drag gesture.
      */}
      <div
        className="absolute inset-0 z-10"
        style={{
        ...
        onMouseLeave={handleOverlayMouseLeave}
      >
        {/* Dashed red rectangle drawn during the drag gesture */}
        {selectionRect && (
          <div
            style={{
              position:        "absolute",
      ...
      {/* ------------------------------------------------------------------ */}
      <div className="absolute top-3 right-3 z-20 flex items-center gap-2">
        {/* Select / Cancel-select toggle */}
        <button
          type="button"
          onClick={toggleSelectMode}
          title={selectMode ? "Exit selection mode" : "Enter selection mode — drag to select properties"}
          className={[
            "flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded shadow border transition-colors",
            selectMode
              ? "bg-red-600 text-white border-red-600 hover:bg-red-700"
      ...
      {/* ------------------------------------------------------------------ */}
      {/* "Delete all selected" button — bottom-centre, visible when ≥1 item  */}
      {/* selected                                                             */}
      {/* ------------------------------------------------------------------ */}
      {selectedIds.size > 0 && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20">
