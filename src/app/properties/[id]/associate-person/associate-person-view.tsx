...
export function AssociatePersonView({ propertyId, propertyName }: Props) {
  ...
  // Search inputs — changes trigger a new query immediately (debounced by
  // React's batching; no extra debounce lib needed at this scale).
  const [nameInput, setNameInput] = useState("");
  const [codeInput, setCodeInput] = useState("");

  // Selected person IDs (checkboxes).
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());

  const [submitting,  setSubmitting]  = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);

  // Only fire the query once the user has typed something, or on first load
  // to show all persons.
  const { data: results, isLoading, isError } = useQuery({
    queryKey: ["person-search", nameInput, codeInput],
    queryFn:  () => searchPersons(nameInput, codeInput),
    // Always enabled — shows all persons on load, refines as user types.
  });
  ...
  const handleAssociate = async () => {
    ...
    try {
      const res = await fetch(
        `/api/properties/${encodeURIComponent(propertyId)}/persons`,
        {
          method:  "POST",
      ...
      // Bust the TanStack Query cache so the Persons tab shows the new data immediately.
      await queryClient.invalidateQueries({ queryKey: ["property-persons", propertyId] });
      // Navigate back to the property detail page, landing on the Persons tab.
  ...
  // Memoised list so toggling checkboxes doesn't re-sort mid-interaction.
