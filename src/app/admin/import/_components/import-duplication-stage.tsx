...
export function ImportDuplicationStage({
   ...
   * null verdict counts zero groups and zero unreadable files: an unticked user
   * would watch the explanations they had just hidden sit open for the whole of
  ...
  return (
    <section className="rounded-xl border border-card-rim bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900">
      ...
      {/* ⚠️ Slice #32.10 — the toggle that stood HERE, above the listing, as a
          ...
          weakened: #32.03's, that the toggle goes with the listing it opens (the
          ...
          explanations are forced open by `nothingToShow` — there the disclosure
          and the region disagreed, and a control whose state contradicts what is
          on screen is worse than no control. That window is now
      ...
      {/* ⚠️ Slice #32.10 — Adrian's order: listing, then this row, THEN the
          ...
          the page can be saved whether or not the explanations are on screen.

          ⚠️ **INSIDE THE SAME `!resultOnly` GUARD AS THE BLOCK IT IS MADE FROM.**
          #32.03 hid the take-away on a clean paused screen because a printed
          copy of explanations nothing broke has no reader, and #32.10 does not revisit
