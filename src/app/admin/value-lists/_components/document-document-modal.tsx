...
async function patchRole(id: string, name: string, description: string): Promise<void> {
  const res = await fetch(`/api/admin/document-document-roles/${id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name, description: description || null }),
  });
...
async function deleteRole(id: string): Promise<void> {
  const res = await fetch(`/api/admin/document-document-roles/${id}`, { method: "DELETE" });
...
function RoleForm({
  ...
  const t  = useTranslations("valueList.documentDocumentRoles");
...
export function DocumentDocumentModal({ onClose }: { onClose: () => void }) {
  const t      = useTranslations("valueList.documentDocumentRoles");
