...
async function fetchPropertyPersonRoles(): Promise<PropertyPersonRoleRow[]> {
  const res = await fetch("/api/admin/property-person-roles");
  if (!res.ok) throw new Error(`Failed to load (${res.status})`);
  const data = await res.json();
  return data.items as PropertyPersonRoleRow[];
}
...
async function addPropertyPersonRole(
  personRoleId: string,
...
async function removePropertyPersonRole(id: string): Promise<void> {
  const res = await fetch(`/api/admin/property-person-roles/${id}`, {
    method: "DELETE",
  });
...
function AddForm({
  ...
  const t = useTranslations("valueList.propertyPersons");
  ...
  const mutation = useMutation({
    mutationFn: () => addPropertyPersonRole(roleId),
    onSuccess: () => {
...
export function PropertyPersonsModal({ onClose }: { onClose: () => void }) {
  const t = useTranslations("valueList.propertyPersons");
  ...
  const listQuery = useQuery<PropertyPersonRoleRow[]>({
    queryKey: ["property-person-roles"],
    queryFn: fetchPropertyPersonRoles,
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => removePropertyPersonRole(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["property-person-roles"] });
  ...
  const alreadyAdded = new Set(
    (listQuery.data ?? []).map((r) => r.personRoleId),
  );
                    ...
                    >
                      ...
                      <td className="max-w-xs px-4 py-2 text-fade dark:text-zinc-400">
                        <span
                          ...
                          title={row.personRoleDescription ?? undefined}
                        >
