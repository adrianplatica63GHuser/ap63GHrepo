 ...
 * stays consistent.  Duplicates are silently ignored via the DB unique index on
 * (principal_object_id, lower(tag)).
 *
 * For property-folder tags (starting with a digit) the following aliases are
 * also stored automatically so searches work regardless of which form is used:
 *
 *   "47per2-225per3per24-2716 prisecaru"  ← original (per-notation)
 *   "47/2-225/3/24-2716 prisecaru"        ← full slash alias
 *   "47/2"                                ← tarla only
 *   "225/3/24"                            ← parcela only
 *   "47/2-225/3/24"                       ← tarla-parcela combined
 *
 * /per(?=\d)/gi only replaces "per" immediately before a digit, so proper
 * names (e.g. "perescu", "prisecaru") are never corrupted.
 */
export async function addEntityTag(
  ...
  if (/^\d/.test(normalised)) {
    // Full slash alias of the original tag
    const slashFull = normalised.replace(/per(?=\d)/gi, "/");
    tags.add(slashFull);

    // Derived tarla / parcela / tarla-parcela tags (always in slash form)
    const pf = parseFolderName(normalised);
    if (pf.isPropertyFolder) {
      ...
      if (tarla)            tags.add(tarla);
      if (parcela)          tags.add(parcela);
      if (tarla && parcela) tags.add(`${tarla}-${parcela}`);
    }
  }

  await db
    .insert(entityTag)
    .values([...tags].map((t) => ({ principalObjectId, tag: t })))
    .onConflictDoNothing();
