#!/usr/bin/env bash
# Claude's side of the test runner protocol (Slice Propus.2). Runs in the device
# VM, over the bridge; the runner itself runs on Windows (runner.ts).
#
...
show() {
  node -e '
    ...
    for (const s of r.steps || []) {
      ...
      if (s.log && s.status !== "passed" && s.status !== "skipped") out.push(`  ${"".padEnd(15)} log: ${s.log}`);
    }
...
wait_for() {
  ...
  while [ "$(date +%s)" -lt "$end" ]; do
    if [ -f "$f" ]; then
      st="$(status_of "$f")"
      case "$st" in passed|failed|error|refused) break ;; esac
    fi
    sleep 5
  done
  if [ ! -f "$f" ]; then
  ...
  show "$f"
  case "$(status_of "$f")" in passed) return 0 ;; failed) return 1 ;; error) return 2 ;; refused) return 3 ;; *) return 4 ;; esac
}
