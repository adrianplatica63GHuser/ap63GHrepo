...
function renderBaseline(lines: string[]): string {
  return (
    [
      ...
      "#   npm run db:verify-rebuild -- --update-baseline",
      "# which never reports a pass, so a re-baseline is always a deliberate act with",
      "# a diff to read. Every line added here is a difference somebody decided not",
      "# to care about.",
      "#",
      ...
      "#    hold different reference data. Measured at Slice #31.01:",
      "#      * lookup_document_type    - 24 rows here vs 27 from the migrations. The",
      "#        file re-introduces `Autorizare`, which migration_043 deleted, keeps",
      "#        `Unclassified` where migration_043 renamed it to `NECLASIFICAT`, omits",
      "#        AUTORIZATIE_CONSTRUIRE / DOCUMENTATIE_CADASTRALA /",
      "#        HOTARARE_ADMINISTRATIVA / CERTIFICAT_DE_MACANENTUR, and 21 of the 22",
      "#        shared keys have a different sort_order, so the dropdown orders",
      "#        differently in a rebuilt project.",
      "#      * lookup_doc_type_person_role - follows from the above (74 vs 67 pairs).",
      "#      * lookup_property_person_role - 4 rows here, 0 from the migrations, which",
      "#        never seed the Property<->Person role whitelist at all.",
      "#    Reconciling them is document-type work and belongs with Slice #29.07, which",
      "#    owns the canonical document-type keys. They are listed here so the check",
      "#    can be green on everything else while making it impossible for the list to",
      "#    grow without somebody re-baselining and being asked why.",
      "",
    ].join("\n") +
    lines.join("\n") +
    "\n"
