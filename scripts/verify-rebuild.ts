...
function renderBaseline(lines: string[]): string {
  return (
    [
      ...
      "#      * lookup_document_type    - the key sets differ by FIVE rows, and",
      "#        every one of them is named below. #29.07 dropped",
      ...
      "#        ONE difference. Slice #34.19 added the four act types",
      "#        (ACT_ADITIONAL, ACT_ALIPIRE, ACT_DEZLIPIRE, ACT_DEZMEMBRARE) to",
      "#        the seed and NOT to the chain, so there are now four more.",
      "#        Six differences remain and all six are deliberate.",
      "#        The four acts are seed-only because the chain seeds document",
      "#        types through migration_072_seed_document_types.sql, which is",
      "#        generated from that block but is an APPLIED migration whose MD5",
      "#        sits in schema_migrations - Apply-Migration.ps1 Step 4 compares",
      "#        it, so regenerating 072 in place would report every developer's",
      "#        database as differing from the repository. Closing this gap is",
      "#        an additive forward migration, which is a slice of its own.",
      ...
      "#        And sort_order differs on 20 of the 40 shared rows (measured",
      "#        again in #34.19, because the previous sentence here said 21 of 26",
      "#        and was written before #29.15 and migration_072; ACT_ADJUDECARE,",
      "#        ACT_CADASTRU, the three rows copied from migration_035 and the",
      "#        fifteen rows #29.15 added already agree, which is the other 20)",
