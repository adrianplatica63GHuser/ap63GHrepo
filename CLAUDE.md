...
- **Migration file workflow — Claude follows these steps every time:**
  ...
  2. **Claude provides** the ready-to-run PowerShell `docker cp` + `docker exec psql` commands (see Local Docker below).
  3. **Adrian runs** those commands in PowerShell to apply the migration to the local Docker database.
  4. **Claude states the filename** (e.g. `src/db/migration_035_seed_doc_types.sql`) so Adrian knows exactly which file to open.
  5. **Adrian pastes** that file's contents into the Supabase SQL Editor to apply it to the cloud database.
