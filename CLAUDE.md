...
- **Reuses the shared primitives** from `src/lib/persons/version-diff.ts` (kept in place — generic despite the `persons/` path) and the shared `VersionNavControls` (`src/components/version-nav-controls.tsx`). i18n keys live under the `document` namespace as `version.*` + a `makeCurrent.*` confirm block.
