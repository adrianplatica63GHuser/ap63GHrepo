...
- Slice #18.10.diviz — Administration → Calculation: upload a 4-section data file (big-polygon Stereo 70 corners / owners + percentages / road side / road width), compute each owner's parcel border (carve a shared road strip along one long side, then divide the rest by ownership fraction; owners 1…N-1 get perpendicular cuts sized to their Final Area, owner N takes the full-height remainder so any percentage rounding lands on N), preview on a Google map + areas table, then create one Property per owner (nickname = owner name) + an optional common-road Property and assign them all to a new PROPERTY-target Group. Scope: HORIZONTAL polygons only, road North/South from the West end. Pure, unit-tested core: `src/lib/calculation/{geometry,parse,compute}.ts` (geometry is conversion-free Stereo 70 maths; compute converts to WGS84 via `transdatRO`). API: `src/app/api/calculation/{preview,commit}/route.ts`. UI: `src/app/admin/calculation/**`. ✅
